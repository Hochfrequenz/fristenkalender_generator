...
class TestFristenkalenderGenerator:
    ...
    @pytest.mark.parametrize("year, nth_day, label, expected", [
    ...
    def test_generate_friste_list_variable_wt(self, year: int, nth_day: int, label: str, expected: date):
        assert FristenkalenderGenerator().generate_fristen_list_variable_wt(year, nth_day, label)[
                   0
               ].date_of_the_frist == expected
    ...
    def test_if_frist_is_in_fristen_calender(self, year: int,
                                             expected: FristWithAttributes):
